...
This repository is a playground for ROC/PR curve and classifier metrics visualization. It consists of a single self-contained HTML file (`index.html`) with no build step or dependencies to install.

## Development

Open `index.html` directly in a browser — no server required. The file loads Plotly and KaTeX from CDNs.

All logic (UI, math, plotting) lives inline in `index.html`. The file uses:
